...
def get_class_function_objs(specs, cname, version = None, includeExt = False):
    ...
    return sorted(objects, key=lambda obj: (float(obj.get('version',"1.0").split(".")[1])*10000) + int(obj.get('ordinal',"100")))
        ...
        for obj in s['objects']:
            ...
            if is_function and match_cls:
                if obj_traits.is_experimental(obj):
                    ...
                    objects.append(obj)
    objects = sorted(objects, key=lambda obj: (float(obj.get('version',"1.0").split(".")[1]) *10000) + int(obj.get('ordinal',"100")))
    exp_objects = sorted(exp_objects, key=lambda obj: (float(obj.get('version',"1.0").split(".")[1])*10000) + int(obj.get('ordinal',"100")))              
