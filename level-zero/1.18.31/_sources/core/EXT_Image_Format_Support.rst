...
*Introduced in Level Zero version 1.15*
...
* Macros
    ...
    * ZE_IMAGE_FORMAT_SUPPORT_EXT_NAME

* Enumerations

    * :ref:`ze-image-format-support-ext-version-t`
    * :ref:`ze-structure-type-t`
        * :ref:`ZE_STRUCTURE_TYPE_IMAGE_FORMAT_SUPPORT_EXT_PROPERTIES <ze-structure-type-t>`
...
By chaining the :ref:`ze-image-format-support-ext-properties-t` structure to :ref:`ze-image-properties-t` when calling :ref:`zeImageGetProperties`\, applications can directly query format support. The Level Zero driver populates the `supported` field based on the image descriptor and device handle provided.
...
To query image format support, create a :ref:`ze-image-format-support-ext-properties-t` structure and chain it to :ref:`ze-image-properties-t` via the pNext member. The driver will populate the `supported` field during the :ref:`zeImageGetProperties` call.
