...
template <> struct WhiteBox<::L0::MemoryManager> : public ::L0::MemoryManager {
    using BaseClass = ::L0::MemoryManager;
};

using MemoryManager = WhiteBox<::L0::MemoryManager>;

template <> struct Mock<MemoryManager> : public MemoryManager {
    Mock();
    virtual ~Mock();

    MOCK_METHOD2(allocateHostMemory, void *(size_t size, size_t alignment));
    MOCK_METHOD3(allocateDeviceMemory,
                 L0::GraphicsAllocation *(L0::Device *device, size_t size, size_t alignment));
    MOCK_METHOD3(allocateManagedMemory,
                 L0::GraphicsAllocation *(L0::Device *device, size_t size, size_t alignment));
    MOCK_METHOD3(allocateManagedMemoryFromFault,
                 L0::GraphicsAllocation *(L0::Device *device, void *buffer, size_t size));
    MOCK_METHOD2(allocateGraphicsMemoryForIsa,
                 PtrOwn<L0::GraphicsAllocation>(PtrRef<const void> isaHostMem, size_t size));
    ...
    L0::GraphicsAllocation *doCreateGraphicsAllocation(L0::Device *device, size_t size,
                                                       size_t alignment);
    PtrOwn<L0::GraphicsAllocation> doCreateGraphicsAllocationForIsa(PtrRef<const void> isaHostMem,
                                                                    size_t size);
