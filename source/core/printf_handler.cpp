...
GraphicsAllocation *PrintfHandler::createPrintfBuffer(Device *device) {
    auto allocation = globalMemoryManager->allocateManagedMemory(
        device, PrintfHandler::printfBufferSize, MemoryConstants::pageSize64k);
    *reinterpret_cast<uint32_t *>(allocation->getHostAddress()) =
        PrintfHandler::printfSurfaceInitialDataSize;
    return allocation;
}

void PrintfHandler::printOutput(PtrRef<FunctionImmutableData> function,
    ...
    *reinterpret_cast<uint32_t *>(printfBuffer->getHostAddress()) =
        PrintfHandler::printfSurfaceInitialDataSize;
}
