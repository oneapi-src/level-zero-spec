...
xe_result_t DeviceImp::canAccessPeer(xe_device_handle_t hPeerDevice, xe_bool_t *value) {
    return XE_RESULT_ERROR_UNSUPPORTED;
}

xe_result_t DeviceImp::copyCommandList(xe_command_list_handle_t hCommandList,
                                       xe_command_list_handle_t *phCommandList) {
    return XE_RESULT_ERROR_UNSUPPORTED;
}

xe_result_t DeviceImp::createCommandList(const xe_command_list_desc_t *desc,
...
xe_result_t DeviceImp::createCommandQueue(const xe_command_queue_desc_t *desc,
...
xe_result_t DeviceImp::createEventPool(const xe_event_pool_desc_t *desc,
                                       xe_event_pool_handle_t *eventPool) {
    *eventPool = EventPool::create(this, desc);
    return XE_RESULT_SUCCESS;
}

xe_result_t DeviceImp::createImage(const xe_image_desc_t *desc, xe_image_handle_t *phImage) {
    auto productFamily = deviceRT->getHardwareInfo().platform.eProductFamily;
...
xe_result_t DeviceImp::createModule(const xe_module_desc_t *desc, xe_module_handle_t *module,
...
xe_result_t DeviceImp::createSampler(const xe_sampler_desc_t *pDesc,
                                     xe_sampler_handle_t *phSampler) {
    return XE_RESULT_ERROR_UNSUPPORTED;
}

xe_result_t DeviceImp::evictImage(xe_image_handle_t hImage) { return XE_RESULT_ERROR_UNSUPPORTED; }

xe_result_t DeviceImp::evictMemory(void *ptr, size_t size) { return XE_RESULT_ERROR_UNSUPPORTED; }

xe_result_t DeviceImp::getApiVersion(xe_api_version_t *version) {
    assert(version != nullptr);
    *version = XE_API_VERSION_1_0;
    return XE_RESULT_SUCCESS;
}

xe_result_t DeviceImp::getComputeProperties(xe_device_compute_properties_t *pComputeProperties) {
    ...
    pComputeProperties->maxGroupSizeX = static_cast<uint32_t>(
        deviceInfo.maxWorkItemSizes[0]); // Note : it doesn't mean that it can be max x max x max
...
xe_result_t DeviceImp::getP2PProperties(xe_device_handle_t hPeerDevice,
                                        xe_device_p2p_properties_t *pP2PProperties) {
    return XE_RESULT_ERROR_UNSUPPORTED;
}

xe_result_t DeviceImp::getMemoryProperties(xe_device_memory_properties_t *pMemProperties) {
    ...
    pMemProperties->deviceAllocCapabilities =
        static_cast<xe_memory_access_capabilities_t>(XE_MEMORY_ACCESS | XE_MEMORY_ATOMIC_ACCESS);
...
xe_result_t DeviceImp::getProperties(xe_device_properties_t *pDeviceProperties) {
    ...
    pDeviceProperties->totalLocalMemSize =
        enableLocalMemory
            ? NEO::DebugManager.flags.HBMSizePerTileInGigabytes.get() * MemoryConstants::gigaByte
            : 0;
...
xe_result_t DeviceImp::getSubDevice(uint32_t ordinal, xe_device_handle_t *phSubDevice) {
    return XE_RESULT_ERROR_UNSUPPORTED;
}

xe_result_t DeviceImp::makeImageResident(xe_image_handle_t hImage) {
    return XE_RESULT_ERROR_UNSUPPORTED;
}

xe_result_t DeviceImp::makeMemoryResident(void *ptr, size_t size) {
    return XE_RESULT_ERROR_UNSUPPORTED;
}

xe_result_t DeviceImp::setIntermediateCacheConfig(xe_cache_config_t CacheConfig) {
    return XE_RESULT_ERROR_UNSUPPORTED;
}

xe_result_t DeviceImp::setLastLevelCacheConfig(xe_cache_config_t CacheConfig) {
    return XE_RESULT_ERROR_UNSUPPORTED;
}

xe_result_t DeviceImp::getImageProperties(const xe_image_desc_t *desc,
...
PtrRef<MOCSMapper> DeviceImp::getMOCSMapper() { return mocsMapper.weakRef(); }
...
xe_result_t DeviceImp::registerCLMemory(cl_context context, cl_mem mem, void **ptr) {
...
xe_result_t DeviceImp::registerCLProgram(cl_context context, cl_program program,
...
xe_result_t DeviceImp::registerCLCommandQueue(cl_context context, cl_command_queue command_queue,
