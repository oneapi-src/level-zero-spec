...
struct FunctionHw : public FunctionImp {
    ...
    void setBufferSurfaceState(uint32_t argIndex, void *address, GraphicsAllocation *alloc) override {
        ...
        EncodeSurfaceState<gfxCoreFamily>::encodeBuffer(surfaceStateAddress.get(), bufferAddressForSsh, bufferSizeForSsh,
    ...
    PtrOwn<Function> clone() const override {
        ...
        if (printfBuffer != nullptr) {
            const auto &it = std::find(cloned->residencyContainer.rbegin(), cloned->residencyContainer.rend(), this->printfBuffer.weakRef().get());
        ...
        if (this->surfaceStateHeapDataSize > 0) {
            cloned->surfaceStateHeapData.rebind(new uint8_t[this->surfaceStateHeapDataSize], this->surfaceStateHeapDataSize);
            memcpy(cloned->surfaceStateHeapData.weakRef().get(), this->surfaceStateHeapData.weakRef().get(), this->surfaceStateHeapDataSize);
            cloned->surfaceStateHeapDataSize = this->surfaceStateHeapDataSize;
        }

        if (this->crossThreadDataSize != 0) {
            cloned->crossThreadData.rebind(new uint8_t[this->crossThreadDataSize], this->crossThreadDataSize);
            memcpy(cloned->crossThreadData.weakRef().get(), this->crossThreadData.weakRef().get(), this->crossThreadDataSize);
            cloned->crossThreadDataSize = this->crossThreadDataSize;
        }

        if (this->dynamicStateHeapDataSize != 0) {
            cloned->dynamicStateHeapData.rebind(new uint8_t[this->dynamicStateHeapDataSize], this->dynamicStateHeapDataSize);
            memcpy(cloned->dynamicStateHeapData.weakRef().get(), this->dynamicStateHeapData.weakRef().get(), this->dynamicStateHeapDataSize);
            cloned->dynamicStateHeapDataSize = this->dynamicStateHeapDataSize;
        }

        if (this->perThreadDataForWholeThreadGroup != nullptr) {
            cloned->perThreadDataForWholeThreadGroup.rebind(static_cast<uint8_t *>(alignedMalloc(perThreadDataSizeForWholeThreadGroupAllocated, 32))); // alignment for vector instructions
            memcpy(cloned->perThreadDataForWholeThreadGroup.weakRef().get(), this->perThreadDataForWholeThreadGroup.weakRef().get(), this->perThreadDataSizeForWholeThreadGroupAllocated);
