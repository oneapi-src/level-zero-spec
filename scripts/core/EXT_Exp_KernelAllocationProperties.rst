...
* Enumerations
    ...
    * ${x}_kernel_get_allocation_properties_exp_version_t

* Structs
    * ${x}_kernel_allocation_exp_properties_t

* Functions

    * ${x}KernelGetAllocationPropertiesExp
...
to store this number of ${x}_kernel_allocation_exp_properties_t must be allocated later.
...
.. parsed-literal::
    ...
    ${x}KernelGetAllocationPropertiesExp(hKernel, &numAllocs, nullptr);
    std::vector<${x}_kernel_allocation_exp_properties_t> allocProps(numAlloc);
    ${x}KernelGetAllocationPropertiesExp(hKernel, &numAllocs, allocProps.data());

    auto toStr = [](${x}_memory_type_t type) {
        switch (type) {
