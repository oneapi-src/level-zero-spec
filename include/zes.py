...
class zes_ras_error_cat_v(IntEnum):
    ...
    PROGRAMMING_ERRORS = auto()                     ## The number of hardware exceptions generated by the way workloads have
                                                    ## programmed the hardware
    DRIVER_ERRORS = auto()                          ## The number of low level driver communication errors have occurred
    COMPUTE_ERRORS = auto()                         ## The number of errors that have occurred in the compute accelerator
                                                    ## hardware
    NON_COMPUTE_ERRORS = auto()                     ## The number of errors that have occurred in the fixed-function
                                                    ## accelerator hardware
    CACHE_ERRORS = auto()                           ## The number of errors that have occurred in caches (L1/L3/register
                                                    ## file/shared local memory/sampler)
    DISPLAY_ERRORS = auto()                         ## The number of errors that have occurred in the display
    MAX = auto()                                    ## The number of categories

class zes_ras_error_cat_t(c_int):
    def __str__(self):
        return str(zes_ras_error_cat_v(value))
...
class zes_ras_state_t(Structure):
    _fields_ = [
        ("category", c_ulonglong * ZES_RAS_ERROR_CAT_MAX)               ## [in][out] Breakdown of error by category
    ]
