...
Other component types can have an arbitrary number of components - for example the number of fans is given by the function ::xetSysmanFanGet().
...
| :---                                   | :---        |
| ::xetSysmanLinkSwitchGetProperties()  | Get the number of ports. |
...
The available promotion modes are described in the enumerator ::xet_standby_promo_mode_t.

## <a name="smw">Firmware</a>
If ::xetSysmanFirmwareGet() returns one or more handles, the following functions can be used to manage firmwares on the device:
...
## <a name="smy">PSU</a>
If ::xetSysmanPsuGet() returns one or more PSU handles, the following functions can be used to access information about each power-supply:
