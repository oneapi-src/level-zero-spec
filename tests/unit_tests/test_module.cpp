...
TEST_P(ModuleCreateBufArg, onlineCompilationModuleTest) {
    ...
    auto src = globalMemoryManager->allocateDeviceMemory(device, 4096u, 4096u);
...
TEST(ModuleCreateSimple, mockedModuleTest) {
    ...
    function.setArgumentValue(0, sizeof(uintptr_t), &addressToPatch);
    auto arg0 = std::find(ctdSearchBeg, ctdSearchEnd, addressToPatch);
    EXPECT_NE(ctdSearchEnd, arg0);

    function.setArgumentValue(0, sizeof(uintptr_t), &clearValue);
    EXPECT_EQ(ctdSearchEnd, std::find(ctdSearchBeg, ctdSearchEnd, addressToPatch));

    function.setArgumentValue(1, sizeof(uintptr_t), &addressToPatch);
