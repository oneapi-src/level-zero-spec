...
def main():
    ...
    add_argument(parser, "icd", "generation of C++ loader files.", True)
    ...
    for idx, section in enumerate(configParser.sections()):
        ...
        if args[section] and util.exists(srcpath):
            ...
            generate_api.generate_cpp(section, namespace, tags, specs, meta)

            if args['icd'] and idx < 1: #todo: generate per-section icd loaders
                generate_icd.generate(namespace, tags, specs, meta)

            if args['cl']:
                compile_api.compile_cpp_source(section, namespace, specs)

            if args['md']:
                generate_docs.generate_md(srcpath, section, tags, meta)
