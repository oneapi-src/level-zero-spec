...
TEST(xeCommandListAppendEventReset, redirectsToObject) {
    Mock<CommandList> commandList;
    Mock<Event> event;

    EXPECT_CALL(commandList, appendEventReset(event.toHandle())).Times(1);

    auto result = xeCommandListAppendEventReset(commandList.toHandle(), event.toHandle());
...
HWTEST_F(CommandListAppendEventReset, addsEventGraphicsAllocationToResidencyContainer) {
    Mock<Device> device;
