...
struct EncodeSurfaceState {
    ...
    static void encodeBuffer(void *dst, void *address, size_t size, uint32_t mocs, bool cpuCoherent) {
        ...
        ss->setSurfaceType((address != nullptr) ? R_SURFACE_STATE::SURFACE_TYPE_SURFTYPE_BUFFER : R_SURFACE_STATE::SURFACE_TYPE_SURFTYPE_NULL);
        ...
        ss->setCoherencyType(cpuCoherent ? R_SURFACE_STATE::COHERENCY_TYPE_IA_COHERENT : R_SURFACE_STATE::COHERENCY_TYPE_GPU_COHERENT);
    ...
    static constexpr uintptr_t getSurfaceBaseAddressAlignment() {
        return 4;
    }
};
