...
* ::XE_MEMORY_ATOMIC_ACCESS - if a device support atomic operations on allocations of the specified type.
...
* ::XE_MEMORY_CONCURRENT_ACCESS - if a device supports concurrent access to allocations of the specified type.
...
* ::XE_MEMORY_CONCURRENT_ATOMIC_ACCESS - if a device supports concurrent atomic operations on allocations of the specified type.
...
- By default, commands are executed in the same order in which they are appended.
  ...
  ::XE_COMMAND_LIST_FLAG_RELAXED_ORDERING.  Reordering is guaranteed to be only occur
...
- By default, commands submitted to a command list are optimized for execution by
  ...
  maximum throughput is desired, applications should use ::XE_COMMAND_LIST_FLAG_MAXIMIZE_THROUGHPUT.
...
Use ::xeDeviceGetSubDevices to confirm subdevices are supported and to obtain a sub-device handle.
...
```c
    ...
    xeDeviceGetSubDevices(hDevice, &subdeviceCount, nullptr);

    xe_device_handle_t* allSubDevices = (xe_device_handle_t*)
        malloc(subdeviceCount * sizeof(xe_device_handle_t));
    xeDeviceGetSubDevices(hDevice, &subdeviceCount, &allSubDevices);

    // Desire is to allocate and dispatch work to sub-device 2.
    assert(subdeviceCount >= 3);
    xe_device_handle_t hSubdevice = allSubDevices[2];
...
### cl_program
Level-Zero modules are always in a compiled state and therefore prior to retrieving an ::xe_module_handle_t from
