...
| ^                   | [ZE_ENABLE_HANDLE_LIFETIME](#v2)          | {**0**, 1}        | Enables the validation level for tracking handle lifetime [future]                |
| ^                   | [ZE_ENABLE_MEMORY_TRACKER](#v3)           | {**0**, 1}        | Enables the validation level for tracking memory lifetime [future]                |
| ^                   | [ZE_ENABLE_THREADING_VALIDATION](#v4)     | {**0**, 1}        | Enables the validation level for multithreading usage [future]                    |
...
The order of the devices reported by the driver can be forced to be consistent by setting the `ZE_ENABLE_PCI_ID_DEVICE_ORDER` environment variable.
