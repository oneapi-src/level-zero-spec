...
A Sysman handle cannot be created for a sub-device - ::${t}SysmanGet() will return error ::${X}_RESULT_ERROR_INVALID_ARGUMENT if a device handle for a 
...
::${t}SysmanPerformanceProfileGetSupported() to get the list avaiable on a given device. In general, the balanced profile (default) is
...
| :---                                 | :---        |
| ::${T}_PERF_PROFILE_BALANCED         | The hardware is configured to strike a balance between compute and memory resources. This is the default profile when the device boots/resets. |
| ::${T}_PERF_PROFILE_COMPUTE_BOUNDED  | The hardware is configured to prioritize performance of the compute units. |
| ::${T}_PERF_PROFILE_MEMORY_BOUNDED   | The hardware is configured to prioritize memory throughput. |
...
| Function                                             | Description |
| :---                                                 | :---        |
| ::${t}SysmanPerformanceProfileGetSupported()         | Get a list of supported performance profiles that can be loaded for this device |
| ::${t}SysmanPerformanceProfileGet()      			   | Get current pre-configured performance profile being used by the hardware |
| ::${t}SysmanPerformanceProfileSet()    			   | Load a pre-configured performance profile |
...
| :---                                      | :---        |
| ::${T}_OC_MODE_OVERRIDE                   | In this mode, a fixed user-supplied voltage (::${t}_oc_config_t.voltageTarget + ::${t}_oc_config_t.voltageOffset) is applied at all times, independent of the frequency request. This is not efficient but can improve stability by avoiding power-supply voltage changes as the frequency changes. |
| ::${T}_OC_MODE_INTERPOLATIVE              | In this mode, a new point is added to the interpolated V-F curve with a user-supplied overclock voltage and frequency (::${t}_oc_config_t.frequency, ::${t}_oc_config_t.voltageTarget). The user can also specify a voltage offset (::${t}_oc_config_t.voltageOffset) that is applied to all points on the interpolated V-F curve, including the new overclock point. This mode is much more efficient than the override mode since lower voltages are used for lower frequencies, but at the expense of small latencies introduced each time the frequency is changed. |
