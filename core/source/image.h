...
struct Image : public _xe_image_handle_t {
    ...
    static Image *fromHandle(xe_image_handle_t handle) { return static_cast<Image *>(handle); }

    inline xe_image_handle_t toHandle() { return this; }
};

using ImageAllocatorFn = Image *(*)();
extern ImageAllocatorFn imageFactory[];

template <uint32_t productFamily, typename ImageType> struct ImagePopulateFactory {
