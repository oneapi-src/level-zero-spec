...
```c
typedef struct _ze_name_handle_t *ze_name_handle_t;
```
</td>
<td>
...
```python
## @brief A brief description...
class ze_name_handle_t(c_void_p):
    pass
```
</td></tr>
<tr><td>
...
zeClsNameFnName(
    ze_cls_handle_t hClsName,   ///< [in] handle to class
...
```python
## @brief A brief description...
_zeClsNameFnName_t = CFUNCTYPE( ze_result_t, ze_cls_handle_t, c_ulong, POINTER(c_ulong), POINTER(c_ulong) )
```
</td></tr>
</table>
...
namespace ze {
    ...
    {
    protected:
        ze_cls_handle_t m_handle;

    public:
        ClsName() = delete;
        ClsName( ze_cls_handle_t handle );
