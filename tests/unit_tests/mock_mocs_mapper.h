...
template <> struct WhiteBox<::L0::MOCSMapper> : public ::L0::MOCSMapper {
    using ::L0::MOCSMapper::MOCSMapper;
};

using MOCSMapper = WhiteBox<::L0::MOCSMapper>;

template <> struct Mock<MOCSMapper> : MOCSMapper {
    Mock() : L0::ult::MOCSMapper(nullptr) {
        // Note : don't add odd values, youngest bit is shared with "EncryptedData" flag
        ON_CALL(*this, getUncachedMOCS).WillByDefault(::testing::Return(0));

        ON_CALL(*this, getFullyCachedMOCS).WillByDefault(::testing::Return(2));

        ON_CALL(*this, getCachedInstructionHeapMOCS).WillByDefault(::testing::Return(4));

        ON_CALL(*this, getCachedStateHeapMOCS).WillByDefault(::testing::Return(6));
