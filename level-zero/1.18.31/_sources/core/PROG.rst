...
descriptor obtained via :ref:`zePhysicalMemGetProperties`\, without the exporting process needing
...
.. parsed-literal::
    ...
    :ref:`ze-external-memory-export-desc-t` exportDesc = {
    ...
    :ref:`ze-physical-mem-desc-t` physMemDesc = {
        :ref:`ZE_STRUCTURE_TYPE_PHYSICAL_MEM_DESC <ze-structure-type-t>`\,
    ...
    ze_physical_mem_handle_t hPhysicalMemory;
    :ref:`zePhysicalMemCreate`\(hContext, hDevice, &physMemDesc, &hPhysicalMemory);

    // Query properties to retrieve the exportable file descriptor
    :ref:`ze-external-memory-export-fd-t` exportFd = {
    ...
    :ref:`ze-physical-mem-properties-t` physMemProps = {
        :ref:`ZE_STRUCTURE_TYPE_PHYSICAL_MEM_PROPERTIES <ze-structure-type-t>`\,
        &exportFd
    };

    :ref:`zePhysicalMemGetProperties`\(hContext, hPhysicalMemory, &physMemProps);
    ...
    :ref:`ze-external-memory-import-fd-t` importFd = {
    ...
    :ref:`ze-physical-mem-desc-t` importMemDesc = {
        :ref:`ZE_STRUCTURE_TYPE_PHYSICAL_MEM_DESC <ze-structure-type-t>`\,
    ...
    ze_physical_mem_handle_t hPhysicalMemoryImport;
    :ref:`zePhysicalMemCreate`\(hContext, hDevice, &importMemDesc, &hPhysicalMemoryImport);

    // Map the imported physical memory into this process's virtual address space
    void* importPtr = nullptr;
    :ref:`zeVirtualMemReserve`\(hContext, nullptr, physicalSize, &importPtr);
    :ref:`zeVirtualMemMap`\(hContext, importPtr, physicalSize, hPhysicalMemoryImport, 0,
...
.. parsed-literal::
    ...
    :ref:`zeVirtualMemReserve`\(hContext, nullptr, physicalSize, &exportPtr);
    :ref:`zeVirtualMemMap`\(hContext, exportPtr, physicalSize, hPhysicalMemory, 0,
        :ref:`ZE_MEMORY_ACCESS_ATTRIBUTE_READWRITE <ze-memory-access-attribute-t>`\);
...
.. parsed-literal::
       ...
       void \*pNext = nullptr;
       :ref:`zeCommandListAppendLaunchKernelWithParameters`\(hCommandList, hKernel, &launchArgs, pNext, nullptr, 0, nullptr);

       // When appending cooperative kernel create cooperative descriptor
       :ref:`ze-command-list-append-launch-kernel-param-cooperative-desc-t` cooperativeDesc = {
           :ref:`ZE_STRUCTURE_TYPE_COMMAND_LIST_APPEND_PARAM_COOPERATIVE_DESC <ze-structure-type-t>`\,
           nullptr,
           true
       };
...
- Kernel object state is updated with new work group size and new arguments, as if separate :ref:`zeKernelSetGroupSize` and :ref:`zeKernelSetArgumentValue` functions were called.
...
.. parsed-literal::
        ...
        :ref:`ze-group-count-t` groupCounts = {1,2,3};
        :ref:`ze-group-size-t` groupSizes = {1,2,3};
        :ref:`zeCommandListAppendLaunchKernelWithArguments`\(hCommandList, hKernel, groupCounts, groupSizes, args, nullptr, nullptr, 0, nullptr);
...
.. note::
    - The host function callback must match the `pfnHostFunction` parameter type accepted by :ref:`zeCommandListAppendHostFunction`\.
    ...
    - In contrast to event-based synchronization - where the application must call :ref:`zeEventHostSynchronize` and block its own thread until the device reaches that point - a host function lets the runtime invoke the host-side work asynchronously, without blocking any application thread.
...
.. parsed-literal::
        ...
        for (int i = 0; i < nFrames; i++) {
            ...
            :ref:`zeCommandListAppendLaunchKernel`\(hImmCmdList, hKernel, &launchArgs[i], nullptr, 0, nullptr);

            // Host function notifies consumer thread that frame i is ready
            :ref:`zeCommandListAppendHostFunction`\(hImmCmdList, onFrameReady, &frames[i], nullptr, nullptr, 0, nullptr);
        }

        // Wait for all GPU work to complete
        :ref:`zeCommandListHostSynchronize`\(hImmCmdList, UINT64_MAX);
...
Timestamps from :ref:`zeCommandListAppendWriteGlobalTimestamp` and kernel timestamp events should not be inferred as equivalent even if reported within identical ranges.
timestampValidBits and kernelTimestampValidBits members of :ref:`ze-device-properties-t`\, must be used to determine the maximum value of the respective timestamps to detect roll over.
...
.. parsed-literal::
       ...
       // Note: :ref:`ZE_STRUCTURE_TYPE_DEVICE_PROPERTIES <ze-structure-type-t>` is deprecated since 1.17 for timer resolution queries.
       // Use :ref:`ZE_STRUCTURE_TYPE_DEVICE_PROPERTIES_1_2 <ze-structure-type-t>` to obtain timerResolution in cycles/sec.
       :ref:`ze-device-properties-t` deviceProperties {};
       deviceProperties.stype = :ref:`ZE_STRUCTURE_TYPE_DEVICE_PROPERTIES_1_2 <ze-structure-type-t>`\;
       :ref:`zeDeviceGetProperties`\(hDevice, &deviceProperties);

       // timerResolution is the timer frequency in cycles/sec (Hz) when
       // stype == :ref:`ZE_STRUCTURE_TYPE_DEVICE_PROPERTIES_1_2 <ze-structure-type-t>`\.
       ...
       :ref:`zeEventCreate`\(hTSEventPool, &tsEventDesc, &hTSEvent);
...
- IPC sharing is one-directional by default. IPC CB Event opened in a different process can be used only for waiting, and the handle must be opened again to observe a new state. Bi-directional sharing, where the opened Event can also be signaled and both processes observe the latest state, is requested with :ref:`ZE_EVENT_COUNTER_BASED_FLAG_IPC_BIDIRECTIONAL <ze-event-counter-based-flags-t>`\. See `IPC sharing`_.
...
Device association
^^^^^^^^^^^^^^^^^^^
Unlike Regular Events, a CB Event's device association is not fixed by the device passed to :ref:`zeEventCounterBasedCreate`\. Its association follows the latest signaling point:

- After creation, the Event is associated with the device passed to :ref:`zeEventCounterBasedCreate`\. This initial association does not restrict on which device the Event may later be signaled.
...
- Waiting on the Event (for example :ref:`zeCommandListAppendWaitOnEvents`\) does not change its association. The waiting device must have peer-to-peer access to the device that last had the Event enqueued for signaling (or, if the Event has not been enqueued for signaling yet, the device it is currently associated with).

.. parsed-literal::
       :ref:`zeEventCounterBasedCreate`\(context, deviceA, &desc, &event); // associated with deviceA
       :ref:`zeCommandListAppendSignalEvent`\(cmdListDeviceB, event);       // re-associates with deviceB (no P2P deviceA<->deviceB required)
       :ref:`zeCommandListAppendWaitOnEvents`\(cmdListDeviceC, 1, &event);  // deviceC must have P2P access to deviceB
...
- :ref:`ZE_EVENT_COUNTER_BASED_FLAG_IPC <ze-event-counter-based-flags-t>` alone selects one-directional sharing
- :ref:`ZE_EVENT_COUNTER_BASED_FLAG_IPC <ze-event-counter-based-flags-t>` combined with :ref:`ZE_EVENT_COUNTER_BASED_FLAG_IPC_BIDIRECTIONAL <ze-event-counter-based-flags-t>` selects bi-directional sharing. :ref:`zeEventCounterBasedCreate` returns :ref:`ZE_RESULT_ERROR_UNSUPPORTED_FEATURE <ze-result-t>` if the Driver doesn't provide it
...
Second process points to the state captured when the IPC handle was obtained, until :ref:`zeEventCounterBasedCloseIpcHandle` is called.
...
- IPC handle obtained with :ref:`zeEventCounterBasedGetIpcHandle` remains valid after the state is replaced. It doesn't have to be obtained again
- Event opened with :ref:`zeEventCounterBasedOpenIpcHandle` remains valid after the state is replaced. It doesn't have to be opened again and stays usable until :ref:`zeEventCounterBasedCloseIpcHandle` is called
...
The same applies to :ref:`zeEventCounterBasedGetDeviceAddress`\: a previously returned memory location and value may be replaced by a signal operation performed in the other process, without any API call in this process, and must be obtained again.
...
- Driver defines a maximum value supported for externally managed counter storage. User must query it via :ref:`zeDeviceGetCounterBasedEventMaxValue` prior to event creation.
- `completionValue` provided in :ref:`ze-event-counter-based-external-sync-allocation-desc-t` must not exceed the value returned by :ref:`zeDeviceGetCounterBasedEventMaxValue`\. Otherwise :ref:`zeEventCounterBasedCreate` returns :ref:`ZE_RESULT_ERROR_INVALID_ARGUMENT <ze-result-t>`\.
- Any value written by the user to `deviceAddress` or `hostAddress` (including explicit signaling writes) must also not exceed the value returned by :ref:`zeDeviceGetCounterBasedEventMaxValue`\. Exceeding this maximum results in undefined behavior.
...
- Signaling is implemented as an atomic increment (atomic_add) of `deviceAddress`. When signaling append calls originate on more than one device (targeting the same aggregate storage), these atomics cross device boundaries. Not all devices support cross-device atomic operations. The user must ensure that atomics are supported between the involved devices, for example by querying :ref:`ZE_DEVICE_P2P_PROPERTY_FLAG_ATOMICS <ze-device-p2p-property-flags-t>` via :ref:`zeDeviceGetP2PProperties` (and/or the cross-device capabilities returned by :ref:`zeDeviceGetMemoryAccessProperties`\). Signaling the same aggregate storage from devices that do not support cross-device atomics results in undefined behavior.
...
- User can programmatically obtain increment value that would work even if underlying append API would be distributed to multiple engines via :ref:`zeDeviceGetAggregatedCopyOffloadIncrementValue` query.
- `completionValue` provided in :ref:`ze-event-counter-based-external-aggregate-storage-desc-t` must not exceed the value returned by :ref:`zeDeviceGetCounterBasedEventMaxValue`\. Otherwise :ref:`zeEventCounterBasedCreate` returns :ref:`ZE_RESULT_ERROR_INVALID_ARGUMENT <ze-result-t>`\.
- User is responsible for ensuring that the value aggregated under `deviceAddress` (initial value plus any number of `incrementValue` additions performed by signaling append calls or by the user directly) does not exceed the value returned by :ref:`zeDeviceGetCounterBasedEventMaxValue` at any point in time. Exceeding this maximum results in undefined behavior.
...
.. parsed-literal::
       ...
       subDeviceProps.stype = :ref:`ZE_STRUCTURE_TYPE_DEVICE_PROPERTIES_1_2 <ze-structure-type-t>`\; // :ref:`ZE_STRUCTURE_TYPE_DEVICE_PROPERTIES <ze-structure-type-t>` deprecated since 1.17
...
in both :ref:`zeMemGetIpcHandle` and :ref:`zeMemGetIpcHandleWithProperties`\. There are two ways to obtain an
...
1. **Direct from a mapped virtual address**: After mapping a physical memory object to a virtual
   address using :ref:`zeVirtualMemMap`\, the mapped virtual address pointer can be passed directly to
   :ref:`zeMemGetIpcHandle` or :ref:`zeMemGetIpcHandleWithProperties`\. Only one physical memory object may
   be associated with a single IPC handle at a time, unless a
   ``:ref:`ze-ipc-phys-mem-handle-range-ext-desc-t`\`` is chained via ``pNext`` to capture a range
...
2. **From a physical memory handle allocated via** ``:ref:`zePhysicalMemCreate`\``: The handle returned
   from :ref:`zePhysicalMemCreate` can be used to obtain an IPC handle via :ref:`zeMemGetIpcHandleWithProperties`\.

When an IPC handle backed by physical memory is opened in the receiving process via
:ref:`zeMemOpenIpcHandle`\, the driver assigns a new virtual address in the importing process that maps
to the underlying physical memory. No prior :ref:`zeVirtualMemReserve` or :ref:`zeVirtualMemMap` call is
...
.. note::
   ...
   it have been closed via :ref:`zeMemCloseIpcHandle` or returned via :ref:`zeMemPutIpcHandle`\. Applications
...
This example demonstrates passing a physical memory handle directly to :ref:`zeMemGetIpcHandle` without
...
.. parsed-literal::
       ...
       :ref:`zeMemPutIpcHandle`\(hContext, hIPC);
       :ref:`zePhysicalMemDestroy`\(hContext, hPhysicalMem);
...
.. parsed-literal::
       ...
       :ref:`zeVirtualMemReserve`\(hContext, nullptr, physicalSize, &mappedPtr);
       :ref:`zeVirtualMemMap`\(hContext, mappedPtr, physicalSize, hPhysicalMem, 0,
           :ref:`ZE_MEMORY_ACCESS_ATTRIBUTE_READWRITE <ze-memory-access-attribute-t>`\);
       ...
       :ref:`ze-ipc-mem-handle-t` hIPC;
       :ref:`zeMemGetIpcHandle`\(hContext, mappedPtr, &hIPC);
       ...
       :ref:`zeMemPutIpcHandle`\(hContext, hIPC);
       :ref:`zeVirtualMemUnmap`\(hContext, mappedPtr, physicalSize);
       :ref:`zeVirtualMemFree`\(hContext, mappedPtr, physicalSize);
       :ref:`zePhysicalMemDestroy`\(hContext, hPhysicalMem);
...
operation (e.g., :ref:`zeCommandListAppendMemoryCopy`\, :ref:`zeCommandListAppendLaunchKernel`\) or in a
residency call (:ref:`zeContextMakeMemoryResident`\).

This cross-device physical memory access is only valid if :ref:`zeDeviceCanAccessPeer` returns success
...
.. parsed-literal::
       ...
       :ref:`zeDeviceCanAccessPeer`\(hDevice0, hDevice1, &canAccess);
       assert(canAccess == ZE_TRUE);

       // Create a physical memory object on hDevice1
       size_t pageSize;
       size_t allocationSize = 1048576; // 1MB
       :ref:`zeVirtualMemQueryPageSize`\(hContext, hDevice1, allocationSize, &pageSize);
       size_t physicalSize = align(allocationSize, pageSize);

       :ref:`ze-physical-mem-desc-t` pmemDesc1 = {
           :ref:`ZE_STRUCTURE_TYPE_PHYSICAL_MEM_DESC <ze-structure-type-t>`\,
           nullptr,
           0, // flags
           physicalSize
       };
       ze_physical_mem_handle_t hPhysicalMem1;
       :ref:`zePhysicalMemCreate`\(hContext, hDevice1, &pmemDesc1, &hPhysicalMem1);

       // Reserve a VA range and map the physical memory from hDevice1 to a VA
       // accessible from hDevice0 (requires P2P support).
       void* peerPtr = nullptr;
       :ref:`zeVirtualMemReserve`\(hContext, nullptr, physicalSize, &peerPtr);
       :ref:`zeVirtualMemMap`\(hContext, peerPtr, physicalSize, hPhysicalMem1, 0,
           :ref:`ZE_MEMORY_ACCESS_ATTRIBUTE_READWRITE <ze-memory-access-attribute-t>`\);

       // Allocate a local buffer on hDevice0 for the copy destination
       void* localPtr = nullptr;
       :ref:`zeMemAllocDevice`\(hContext, &deviceAllocDesc, physicalSize, 0, hDevice0, &localPtr);

       // peerPtr can now be used in a command list on hDevice0 via P2P access
       :ref:`zeCommandListAppendMemoryCopy`\(hCommandList0, localPtr, peerPtr, physicalSize,
           nullptr, 0, nullptr);
       :ref:`zeCommandListClose`\(hCommandList0);
       :ref:`zeCommandQueueExecuteCommandLists`\(hCommandQueue0, 1, &hCommandList0, nullptr);
       :ref:`zeCommandQueueSynchronize`\(hCommandQueue0, UINT64_MAX);

       // Cleanup
       :ref:`zeVirtualMemUnmap`\(hContext, peerPtr, physicalSize);
       :ref:`zeVirtualMemFree`\(hContext, peerPtr, physicalSize);
       :ref:`zePhysicalMemDestroy`\(hContext, hPhysicalMem1);
       :ref:`zeMemFree`\(hContext, localPtr);
...
- Accesses to VA regions backed by physical memory on another device are serviced via P2P, and
  are only valid if :ref:`zeDeviceCanAccessPeer` returns success for those two devices.
...
.. parsed-literal::
       ...
       :ref:`zeVirtualMemQueryPageSize`\(hContext, hDevice0, halfSize, &pageSize);
       size_t segmentSize = align(halfSize, pageSize);
       size_t totalSize = segmentSize * 2;

       // Create physical memory on hDevice0 (local segment)
       :ref:`ze-physical-mem-desc-t` pmemDesc0 = {
           :ref:`ZE_STRUCTURE_TYPE_PHYSICAL_MEM_DESC <ze-structure-type-t>`\, nullptr, 0, segmentSize
       };
       ze_physical_mem_handle_t hPhysMem0;
       :ref:`zePhysicalMemCreate`\(hContext, hDevice0, &pmemDesc0, &hPhysMem0);

       // Create physical memory on hDevice1 (remote segment, accessed via P2P)
       :ref:`ze-physical-mem-desc-t` pmemDesc1 = {
           :ref:`ZE_STRUCTURE_TYPE_PHYSICAL_MEM_DESC <ze-structure-type-t>`\, nullptr, 0, segmentSize
       };
       ze_physical_mem_handle_t hPhysMem1;
       :ref:`zePhysicalMemCreate`\(hContext, hDevice1, &pmemDesc1, &hPhysMem1);

       // Reserve a contiguous VA range large enough to hold both segments
       void* vaBase = nullptr;
       :ref:`zeVirtualMemReserve`\(hContext, nullptr, totalSize, &vaBase);

       // Map first half of the VA range to hDevice0's physical memory (local)
       :ref:`zeVirtualMemMap`\(hContext, vaBase, segmentSize, hPhysMem0, 0,
           :ref:`ZE_MEMORY_ACCESS_ATTRIBUTE_READWRITE <ze-memory-access-attribute-t>`\);

       // Map second half of the VA range to hDevice1's physical memory (P2P)
       void* vaSecondHalf = (uint8_t*)vaBase + segmentSize;
       :ref:`zeVirtualMemMap`\(hContext, vaSecondHalf, segmentSize, hPhysMem1, 0,
           :ref:`ZE_MEMORY_ACCESS_ATTRIBUTE_READWRITE <ze-memory-access-attribute-t>`\);
       ...
       :ref:`zeCommandListAppendMemoryFill`\(hCommandList0, vaBase, &pattern, sizeof(pattern),
       ...
       // Cleanup
       :ref:`zeVirtualMemUnmap`\(hContext, vaBase, segmentSize);
       :ref:`zeVirtualMemUnmap`\(hContext, vaSecondHalf, segmentSize);
       :ref:`zeVirtualMemFree`\(hContext, vaBase, totalSize);
       :ref:`zePhysicalMemDestroy`\(hContext, hPhysMem0);
       :ref:`zePhysicalMemDestroy`\(hContext, hPhysMem1);
