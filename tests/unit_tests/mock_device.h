...
template <> struct WhiteBox<::L0::Device> : public ::L0::Device {};

using Device = WhiteBox<::L0::Device>;

template <> struct Mock<Device> : public Device {
    Mock();
    virtual ~Mock();

    MOCK_METHOD2(canAccessPeer, xe_result_t(xe_device_handle_t hPeerDevice, xe_bool_t *value));
    ...
    MOCK_METHOD2(createEventPool,
                 xe_result_t(const xe_event_pool_desc_t *desc, xe_event_pool_handle_t *eventPool));
    MOCK_METHOD2(createImage, xe_result_t(const xe_image_desc_t *desc, xe_image_handle_t *phImage));

    MOCK_METHOD3(createModule, xe_result_t(const xe_module_desc_t *desc, xe_module_handle_t *module,
                                           xe_module_build_log_handle_t *buildLog));
    MOCK_METHOD2(createSampler,
                 xe_result_t(const xe_sampler_desc_t *pDesc, xe_sampler_handle_t *phSampler));
    MOCK_METHOD1(evictImage, xe_result_t(xe_image_handle_t hImage));
    MOCK_METHOD2(evictMemory, xe_result_t(void *ptr, size_t size));
    MOCK_METHOD1(getApiVersion, xe_result_t(xe_api_version_t *version));
    MOCK_METHOD1(getComputeProperties,
                 xe_result_t(xe_device_compute_properties_t *pComputeProperties));
    ...
    MOCK_METHOD2(getSubDevice, xe_result_t(uint32_t ordinal, xe_device_handle_t *phSubDevice));
    MOCK_METHOD1(makeImageResident, xe_result_t(xe_image_handle_t hImage));
    MOCK_METHOD2(makeMemoryResident, xe_result_t(void *ptr, size_t size));
    ...
    MOCK_METHOD2(getImageProperties,
                 xe_result_t(const xe_image_desc_t *desc, xe_image_properties_t *pImageProperties));
    ...
    MOCK_METHOD3(registerCLProgram,
                 xe_result_t(cl_context context, cl_program program, xe_module_handle_t *phModule));
    MOCK_METHOD3(registerCLCommandQueue,
                 xe_result_t(cl_context context, cl_command_queue command_queue,
