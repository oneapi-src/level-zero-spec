...
::ze_device_handle_t and a pointer to a ::zet_debug_config_t object
...
ancestor device results in ::ZE_RESULT_ERROR_DEVICE_ACCESS.
...
::ZET_DEBUG_THREAD_ALL to resume an entire debug session.
...
::ZE_RESULT_ERROR_INVALID_ARGUMENT (see below for an exception to this).
...
```c
    ...
    errcode = zetDebugInterrupt(session, ZET_DEBUG_THREAD_ALL);
    if (errcode)
        return errcode;

    ...

    errcode = zetDebugResume(session, ZET_DEBUG_THREAD_ALL);
    if (errcode)
        return errcode;
```

After interrupting one or all threads, the tool needs to wait for the
corresponding ::ZET_DEBUG_EVENT_EXCEPTION event.  Note that there may be
...
To read and write the register state, use the ::zetDebugReadState and
::zetDebugWriteState function, respectively.  They take a
::zet_debug_session_handle_t, a thread handle, an offset into the
register state area, an access size in bytes, and an input or output
buffer.

The register state area starts with a ::zet_debug_state_t descriptor
...
starting at offset ::zet_debug_state_t.headerSize of the register state
...
```c
    ...
    for (sec = 0; sec < state.numSec; ++i) {
        zet_debug_state_section_t section;
        uint64_t offset;
