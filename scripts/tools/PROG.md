...
To wait for events, the tool passes the ::${t}_debug_session_handle_t, a
...
timeout of ::${T}_DEBUG_TIMEOUT_INFINITE waits indefinitely.  On success,
the API provides the size of the topmost event in bytes.  If the timeout
expires, ::${X}_RESULT_NOT_READY is returned.
...
```c
    ...
    errcode = ${t}DebugWaitForEvent(session, ${T}_DEBUG_TIMEOUT_INFINITE,
                                    ${T}_DEBUG_WAIT_NONE, &size);
    if (errcode)
        return errcode;
    ...
    errcode = ${t}DebugReadEvent(session, size, pbuffer);
    if (errcode) {
        if (pbuffer != buffer)
            free(pbuffer);
        return errcode;
...
A debug event is described by the ::${t}_debug_event_t structure.  It
contains:
...
fields.
  ...
  * ::${T}_DEBUG_EVENT_PROCESS_ENTRY: the host process created one of more
  ...
  * ::${T}_DEBUG_EVENT_EXCEPTION: the thread stopped due to a device
    ...
    * The device-specific exception code.  If thread ==
      ::${T}_DEBUG_THREAD_ALL, this may be a summary exception code,
    ...
    * The stack pointer.  This will be zero if thread ==
      ::${T}_DEBUG_THREAD_ALL or if the implementation does not use a
      stack.
...
To read and write uncompressed memory, call the ::${t}DebugReadMemory and
...
::${t}DebugReadCompressedMemory and ::${t}DebugWriteCompressedMemory
